...
export async function POST(req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  if (!session) {
    return NextResponse.redirect(new URL("/topics?error=unauth", req.url), 303);
  }
  if (session.role === "EMPLOYEE") {
    return NextResponse.redirect(new URL("/topics?error=forbidden", req.url), 303);
  }

  const { id } = await ctx.params;
  const idNum = Number(id);
  if (!Number.isInteger(idNum)) {
    return NextResponse.redirect(new URL("/topics?error=badid", req.url), 303);
  }
  ...
  try {
    ...
    return NextResponse.redirect(new URL(`/topics/${id}/delete/edit?error=update_failed`, req.url), 303);
  }
}
